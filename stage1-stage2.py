...
def main(args):
    ...
    model = models.create("ft_net_inter_resnet50",
    ...
    train_transformer = T.Compose(train_transformer)
                ...
                #and in each round, 3 times for training intra and 2 times for training inter model(120 epoch for intra and 8- epochs for inter in total)
    for cluster_epoch in range(args.cluster_epochs):
        ...
        #len of cluster result for first time is 6 for market1501 dataset
        cluster_datasets = [
           # datasets.create("cluster", osp.join(args.data_dir, args.dataset),
           datasets.create("cluster", args.data_dir,
        ...
        # Trainer
        trainer = IntraCameraSelfKDTnormTrainer(model,
                                                criterion,
                                                soft_criterion,
...
#change
        cluster_result = get_inter_cam_cluster_result_without_tnorm(model,
                                                            train_loader,
                                                            args.eph_stage2,
                                                            args.linkage,
                                                            mix_rate,
        ...
        cluster_dataset = datasets.create(
            #change this line to be able to read dataset from data-dir
        ...
        criterion_triple = TripletLoss(margin=args.margin).to(device)
        #criterion_triple = TripletLoss(margin=args.margin).cuda()
        criterion_soft = SoftEntropy().to(device)
        criterion_triple_soft = SoftTripletLoss(margin=None).to(device)
        # criterion_soft = SoftEntropy().cuda()
        # criterion_triple_soft = SoftTripletLoss(margin=None).cuda()
        # Optimizer
        ...
        optimizer = torch.optim.SGD(param_groups, momentum=0.9)
        # Trainer
        trainer = InterCameraSelfKDTNormTrainer(
            model,
            criterion_entropy,
            criterion_triple,
            criterion_soft,
            criterion_triple_soft,
            warm_up_epoch=args.warm_up,
        )
