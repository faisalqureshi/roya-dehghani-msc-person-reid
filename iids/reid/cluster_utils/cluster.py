...
"""this function takes resnet50 model and gives feature vector 2048 for each img"""
def extract_features_per_cam(model, data_loader, norm=True):
    ...
    for imgs, fnames, _, camid in tqdm(data_loader):
        ...
        with torch.no_grad():
            ...
            """fname convert each 2048 vector to one number"""
            fnorm = torch.norm(outputs, p=2, dim=1, keepdim=True)
            #print("fnorm", fnorm.shape)
            """this line expand one number to 2048 numbers"""
            norm_outputs = outputs.div(fnorm.expand_as(outputs))
            #print("norm_output", norm_outputs.shape)
            """len(outputs) is the number of images in each camera"""
        ...
        if norm:
            ...
            #print("per_cam_fname[0]",per_cam_fname[5])
        else:
            """if norm=false, use norm-outputs and outputs"""
        ...
        """per_cam_features_norm contains 0: [tensor([0.0220, 0.0205, 0.0215,  ..., 0.0215, 0.0202, 0.0212]... and len(per_cam_features_norm) is 6
        per_cam_fname contains {0: ['00001368_00_0000.jpg', '00001368_00_0001.jpg', '00000738_00_0000.jpg',...] and len(per_cam_fname) is 6"""      
       
        
        
    if norm:
        #len(per_cam_features_norm[0]=2017 , len(per_cam_fname[0]=2017 , len(per_cam_fname[1], len(per_cam_fname[2]1709 2707 920 2338 3245
        #len per_cam_features_norm[0][0] 2048 len per_cam_features_norm[1][0]2048 len per_cam_features_norm[2][0] 2048 2048 2048 2048 and len per_cam_fname[0][0] 20 [1][0] 20 [2][0] 20 20 20 20
        #print("len per_cam_features_norm",len(per_cam_features_norm[0][0]),len(per_cam_features_norm[1][0]),
              #len(per_cam_features_norm[2][0]),len(per_cam_features_norm[3][0]),len(per_cam_features_norm[4][0]),
                                               # len(per_cam_features_norm[5][0]))
        #print("len per_cam_fname",len(per_cam_fname[0][0]),len(per_cam_fname[1][0]),
              #len(per_cam_fname[2][0]),len(per_cam_fname[3][0]),len(per_cam_fname[4][0]),
                                               # len(per_cam_fname[5][0]))
        
        
        
        """store  in dataframe"""
        # d = dict( A =per_cam_fname[0],B= per_cam_features_norm[0], C=per_cam_fname[1], D= per_cam_features_norm[1],
        #         q =per_cam_fname[2],w= per_cam_features_norm[2], e=per_cam_fname[3], r= per_cam_features_norm[3],
        #         a =per_cam_fname[4],s= per_cam_features_norm[4], f=per_cam_fname[5], h= per_cam_features_norm[5],
        #        )
        ...
        #print(data)
        #data.to_csv('/home/roya/IIDS/example/per_cam_fname_and_features_norm.csv')
        return per_cam_features_norm, per_cam_fname
    
    else:
        #train_pd=pd.DataFrame(per_cam_features_norm)
...
"""this function calcluates distance between features to do clustering"""
