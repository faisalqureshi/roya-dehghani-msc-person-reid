...
def extract_cnn_feature_specific(model, inputs, camid, norm=True):
    def fliplr(img):
        '''flip horizontal'''
        inv_idx = torch.arange(img.size(3) - 1, -1, -1).long()  # N x C x H x W
        img_flip = img.index_select(3, inv_idx)
    ...
    for i in range(2):
        ...
        inputs2 = inputs.cuda()
        if hasattr(model, "module"):
            outputs = model.module.backbone_forward(inputs2, camid, True)
        else:
            outputs = model.backbone_forward(inputs2, camid, True)
        outputs = outputs.view(outputs.size(0), outputs.size(1))
